...
saveRDS(com_df, "X:/Spatial Stat/WASH Cholera/clean_repo/results/poisson_results.RDS")

ggplot(data = com_df, aes(x= est, y=var, color= source))+
  geom_vline(xintercept = 1)+
  geom_hline(yintercept = 4.5, color= "grey", linetype= 2)+ 
  ...
  annotate("text", x = c(.89), y=c(1,4.5), label = c("Risk Factor", "Protective Factor"), angle= 90, size= 3, hjust= -.4)
  
ggsave("X:/Spatial Stat/WASH Cholera/clean_repo/results/Figure_2_Poisson_results.jpg", width = 6, height = 4, units = "in", dpi= 400)
