...
main1<- ggplot(data= full.incidence.data, aes(Predicted, Observed, color= cv.number))+ 
  ...
  theme(legend.position = "none", panel.grid = element_line(linetype = 3, size = .5), panel.grid.minor = element_blank())+#, aspect.ratio = 1)+
...
ggsave("X:/Spatial Stat/WASH Cholera/combined_models_importance2_dec31_2021.jpeg", plot= plot3.v1, width = 6, height = 8, units = "in", dpi= 500)
